import { authorization, skipAuth, isAuth } from "./authorization.ts";
import { CHAT_UI } from "./UI.ts";
...
const templateMessage: HTMLTemplateElement | null =
  document.querySelector("#tmp__message");
...
function virtualize(this: any) {
  ...
  if (cordWindow < 40 && breack) {
    ...
    CHAT_UI.WINDOW!.scrollTop =
      CHAT_UI.WINDOW!.scrollTop + this.scrollHeight - scrollHeightLast;
    setTimeout(() => {
      breack = !breack;
    }, 1000);
  }
...
function loadHistory() {
  const messages = JSON.parse(
    localStorage.getItem("historyMesseges") as string
  );
  const messagesPiece = messages.slice(lastMessegeIndex, lastMessegeIndex + 20);

  CHAT_UI.WINDOW!.append(
    ...messagesPiece.map((message: Message.RootObject) => {
...
function createMessage(message: Message.RootObject) {
  ...
  const isMine = isMyMessage(message.user.email)
    ? "my-message"
    : "other-message";
